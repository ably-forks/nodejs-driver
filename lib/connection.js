...
Connection.prototype.bindSocketListeners = function() {
  ...
  this.netClient.removeAllListeners('clientError');
  ...
  this.protocol = new streams.Protocol({ objectMode: true, logger: this });
...
Connection.prototype.open = function (callback) {
  ...
  this.netClient.once('clientError', function (err) {
    self.errorConnecting(err, false, callback);
  });
...
Connection.prototype.sendStream = function (request, options, callback) {
  ...
  //noinspection JSUnresolvedVariable
  this.streamHandlers[request.streamId] = {
    callback: callback,
...
Connection.prototype.getWriteCallback = function (request, options, callback) {
  ...
  return (function writeCallback (err) {
    if (err) {
      ...
      self.log('error', 'Error sending stream #' + request.streamId + ' to ' + self.endpoint + '; err = ' + err.message);
      delete self.streamHandlers[request.streamId];
      self.freeStreamId(request.streamId);
      return callback(err);
    }
...
Connection.prototype.idleTimeoutHandler = function () {
  ...
  var deadlineTimer = setTimeout(function() {
    var err = new Error('Connection timeout error');
    self.log('warning', 'Heartbeat timeout error', err);
    self.emit('idleRequestTimeout', err);
  }, this.options.pooling.heartBeatDeadline);
  this.sendStream(new requests.QueryRequest(idleQuery), utils.emptyObject, function (err) {
    self.sendingIdleQuery = false;
    var elapsedTime = Date.now() - requestTime;
    clearTimeout(deadlineTimer);
    if (!err) {
      self.log('verbose', 'Heartbeat request latency ' + elapsedTime + ' ms');
      self.emit('idleRequest', elapsedTime);
      return;
    }
    self.log('warning', 'Received heartbeat request error', err);
    self.emit('idleRequestError', err, elapsedTime);
  });
};
