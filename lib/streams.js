...
function Protocol (options) {
  ...
  this._id = String(Math.random()).slice(2);
}
...
Protocol.prototype.readItems = function (chunk) {
  ...
  this.logger.log('verbose', 'readItens: id = ' + this._id + '; length: ' + chunk.length + '; chunk:\n' + hexy.hexy(chunk.slice(0, 32)));
  if (this.version === 0) {
    ...
    this.logger.log('verbose', 'readItens: id = ' + this._id + '; version: ' + this.version);
  ...
  if (this.headerChunks.byteLength !== 0) {
    ...
    if (this.headerChunks.byteLength + chunk.length < this.headerSize) {
      ...
      this.logger.log('verbose', 'readItens: id = ' + this._id + '; incomplete header; length: ' + this.headerChunks.byteLength);
      return;
    }
    var headerChunks = Buffer.concat(this.headerChunks.parts, this.headerSize);
    currentHeader = FrameHeader.fromBuffer(headerChunks);
    offset = this.headerSize - this.headerChunks.byteLength;
    this.logger.log('verbose', 'readItens: id = ' + this._id + '; complete header; offset: ' + offset + '; headerChunks:\n' + hexy.hexy(headerChunks.slice(0, 32)) + '; currentHeader:\n' + util.inspect(currentHeader));
    this.clearHeaderChunks();
  }
  var items = [];
  while (true) {
    if (!currentHeader) {
      if (this.headerSize > chunk.length - offset) {
        ...
        this.logger.log('verbose', 'readItens (in loop): id = ' + this._id + '; incomplete header; length: ' + this.headerChunks.byteLength);
        break;
      }
      //read header
      currentHeader = FrameHeader.fromBuffer(chunk, offset);
      this.logger.log('verbose', 'readItens (in loop): id = ' + this._id + '; complete header; offset: ' + offset + '; chunk:\n' + hexy.hexy(chunk.slice(0, 32)) + '; currentHeader:\n' + util.inspect(currentHeader));
      offset += this.headerSize;
      this.clearHeaderChunks();
    }
    //parse body
    var remaining = chunk.length - offset;
    this.logger.log('verbose', 'readItens (in loop): id = ' + this._id + '; parse body; offset: ' + offset + '; remaining: ' + remaining + '; currentHeader.bodyLength: ' + currentHeader.bodyLength + '; this.bodyLength: ' + this.bodyLength);
    if (currentHeader.bodyLength <= remaining + this.bodyLength) {
      this.logger.log('verbose', 'readItens (in loop): id = ' + this._id + '; emitting complete frame; offset: ' + offset + '; chunk:\n' + hexy.hexy(chunk.slice(0, 32)));
      ...
      if (remaining > 0) {
        ...
        this.logger.log('verbose', 'readItens (in loop): id = ' + this._id + '; emitting incomplete frame; offset: ' + offset + '; chunk:\n' + hexy.hexy(chunk.slice(0, 32)));
