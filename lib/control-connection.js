...
ControlConnection.prototype.getAddressForPeerHost = function (row) {
  ...
  var peer = row['peer'];
  var bindAllAddress = '00000000';
  if (!address) {
    this.log('error', util.format('No rpc_address found for host %s in %s\'s peers system table. %s will be ignored.', utils.toIpString(peer), this.host.address, utils.toIpString(peer)));
    return null;
  }
  if (address.toString('hex') === bindAllAddress) {
    ...
    address = peer;
  } else if (this.options.preferPeerAddress) {
    address = peer;
  }
